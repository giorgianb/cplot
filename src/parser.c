...
void
expression_destroy (const expression_t e)
{
  switch (e.type)
    {
...
expression_t
get_nth_level_expression (FILE * const stream, const int n)
{
  switch (n)
    {
    case 0:
	...
	switch (tok.type)
	  ...
	  case TOKEN_ARITHMETIC_OPERATOR:
		...
		if (paren.type != TOKEN_ARITHMETIC_OPERATOR
		    || paren.operator != ')')
		  {
		    token_destroy (paren);
...
expression_t
next_expression (FILE * const stream)
{
